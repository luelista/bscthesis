 ...
 *  ... except images, send images only on the "cheap" subflow (e.g. wifi)
*/
...
IF (!RQ.EMPTY) {
	...
	IF (!notSentOnSbfs.EMPTY) {
		PRINT("resending on all, prio=%d", RQ.TOP.USER);
...
IF (!Q.EMPTY) {
	...
	IF (Q.TOP.USER != R3) {
		PRINT(". . sent %d segments", R4);
		PRINT("new prio=%d", Q.TOP.USER);

		SET(R3, Q.TOP.USER);
		SET(R4, 0);

		PRINT("Number of Backup Sbf %u", SUBFLOWS.FILTER(s => s.IS_BACKUP).COUNT);
		FOREACH(VAR sbf IN SUBFLOWS) {
			PRINT("subflow %d", sbf.ID);
			PRINT(".   is_backup=%d", sbf.IS_BACKUP);
			PRINT(".   rtt=%d", sbf.RTT);
		}
	} 
	SET(R4, R4 + 1);
	...
	IF (Q.TOP.USER >= SKB_CONTENT_IMAGE) {
		/* images should be only sent on the "cheap" subflow */
		
		/* the expensive subflow is marked as "BACKUP" and filtered out */
