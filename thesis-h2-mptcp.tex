...
The rule-based scheduler by \textcite{RBS} is a Multipath TCP scheduler, which runs a small program for every scheduling decision. These programs can be written in the RBS scripting language and are compiled to a byte code, which is executed by a virtual machine in the scheduler. The possibility of dynamically loading the scheduling algorithms at runtime, without having to recompile the Linux kernel, allows us to evaluate different scheduling approaches quickly.

%text from implementation: The scheduler algorithms are implemented in a scripting language. The scripts are just-in-time compiled and interpreted by the Rule Based Scheduler, a byte-code based virtual machine running in the MPTCP implementation in the Linux kernel.
...
The resources required for the first layout cycle will be handled by a scheduler with emphasis on low latency, %
%latency critical stuff -> redundant if lossy
like a redundant scheduler on lossy connections. The redundant scheduler sends all segments out on all paths to minimize delays caused by packet loss.
%latency critical stuff -> send on backup if backup has lower rtt
\todo{...or send on low-rtt despite IS_BACKUP}
...
To get information about the contents of the HTTP/2 application data, the cooperation of the web server is required. Multiple web servers have been evaluated and based on the criteria set out below, a suitable basis for the implementation has been selected.

To be at all useful, the server must support HTTP/2, and support it with at least one common browser as client. As we want to implement low-level modifications in the web server, the source code has to be available, and the license must permit modifications. This is a strictly necessary criterion, but luckily, most available web servers are permissively open source licensed under Apache, MIT or similar licenses. To make the work more relevant for practical use, a popular web server should be used. Therefore, we started our evaluation with \textit{Apache HTTP Server} and \textit{nginx}, the two commonly used open source HTTP servers. We found that Apache HTTP Server internally uses the third-party library \textit{libnghttp} for its HTTP/2 support, which contains the reference HTTP/2 server implementation \textit{nghttpd}, which we also evaluated and finally selected.
...
\begin{lstlisting}[label={lst:nghttpd_select_ruleset},caption={Selecting a RBS Script in the HTTP/2 Connection Setup Method of Nghttpd},numbers=left,firstnumber=281,linebackgroundcolor={\lstLinesAdded{284-295}}]
...
608        return stream->priority_file_type;
	[...]
614    }
615  }
\end{lstlisting}
...
\begin{lstlisting}[label=lst:sch_netem,caption={Modified version of Linux kernel 4.4, \code{net/sched/sch_netem.c}, line 398ff}]
/*
...
\begin{lstlisting}[label=lst:sched_eval_a,caption={RBS Scheduler Script for Evaluation A (``Bad Wi-Fi'')},numbers=left]
/*
