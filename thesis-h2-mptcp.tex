...
The current reference implementation\footnote{\url{http://multipath-tcp.org/}} is based on the Linux kernel and maintained by a group at the Université Catholique de Louvain\cite{multipathtcp}. 
%other implementations
The most widely deployed implementation is found in Apple's XNU kernel \cite{Hesmans2016}, used in the macOS and iOS operating systems. It is used mainly for the personal assistant software, which keeps a backup connection over cellular network in case the Wi-Fi connection breaks down \cite{AppleSupportMPTCP}. 
%https://support.apple.com/en-us/HT201373
Another implementation is developed as a patch for the FreeBSD kernel at Swinburne University \footnote{\url{http://caia.swin.edu.au/newtcp/mptcp/}}. The first implementation of Multipath TCP was developed as a user-space proxy\footnote{\url{https://www.ietf.org/mail-archive/web/multipathtcp/current/msg01934.html}} by Hampel at Bell Labs. All aforementioned software is open source. There is also one known closed-source implementation by Citrix Networks in their load balancer firmware \cite{Eardley2013}.


\subsection{Socket API}

%application api's
In the current Linux implementation, the only user-space control is via \code{sysctl} parameters, allowing the system administrator to enable or disable Multipath TCP for all applications, and to change a number of high-level parameters globally. If enabled, Multipath TCP is used for all TCP connections and is transparent to the application. Application developers can't control details of the Multipath TCP behaviour. However, different APIs have been proposed and implemented to give individual, MPTCP-aware applications some control over subflow details. Scharf and Ford \cite{Scharf2013} present a "`Basic API for MPTCP-Aware Applications"', enabling applications to choose whether MPTCP should be used on a given socket, restrict the interfaces to use for establishing paths and query the subflows and their addresses. 
...
During our evaluations with Mininet and comparisions on real hardware, we noticed that in Mininet the TCP Small Queue throttling never got engaged. 
