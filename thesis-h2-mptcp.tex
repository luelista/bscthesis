...
With Multipath TCP, we can do this even in a more fine-grained way, 


\textcite{nowlan2012unorderedtcp} \todo{ inhalt kurz zusammenfassen}
...
through the different components of the Nghttpd web server. Nghttpd uses 
...
\begin{figure}
	...
	\caption{The annotated data's path through the application layer}
...
The response to a request is generated in \code{Http2Handler::submit_response}
 and subsequently enqueued in Libnghttp's internal buffer of outbound items 
by calling \code{nghttp2_submit_response}. Another event watcher is \code{
write_tls}, which is run whenever the socket signalizes that data can be sent 
out. It serves two purposes: if data to be sent is available in \code{wb_}, 
it sends it out over OpenSSL's \code{SSL_write}. \code{wb_} is an opaque 
buffer of octets, which means no information about the nature of the data 
inside is available at this point. It is used to decouple the generation of 
HTTP/2 encoded data by Libnghttpd from the generation of TLS records by 
OpenSSL. If this wasn't done, each TLS record could contain at most a single 
HTTP/2 frame, which would be inefficient. If \code{wb_} is empty, \code{write_
tls} runs \code{fillwb}, which fetches data to send from \code{nghttp2_session
_mem_send}, and enqueues it in \code{wb_}.
We extend this process with a linked list (shown in Listing \ref{lst:nghttpd-
linkedlist-headers}) which can store metadata about the octets enqueued in 
\code{wb_}, along with the amount of octets this metadata is valid for. \code{
fillwb} enqueues and \code{write_tls} dequeues the metadata in lockstep with 
the data. 

\begin{lstlisting}[style=C_Code,label=lst:nghttpd-linkedlist-headers,caption={Declaration of the data structure for metadata storage (\code{mptcp_rbs.h})}]
...
select what information is used as the hint. With our optimized scheduler 
...
\begin{lstlisting}[style=C_Code,numbers=left,label=lst:nghttpd-get_skb_property,caption={The \code{get_skb_property} function added to Nghttpd (\code{HttpServer.cc})}]
...
\begin{lstlisting}[style=RBS,label=lst:scheduling-content-type-decl,caption={Possible values of the USER field if simplified content types are used as hints.}]
