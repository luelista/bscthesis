...
An evaluation is performed in simulated and real-world environments over LTE and WiFi, demonstrating that better trade-offs between (perceived) speed and network load can be achieved by utilizing the additional information.
...
\paragraph{Available Variables}
The \code{Q} and \code{RQ} variables provide access to packet list objects, 
...
script. Table \ref{tab:skb-attrs} lists the attributes and methods providing 
information about a packet.
The \code{SUBFLOWS} variable contains the list of all subflows. In table 
\ref{tab:subflow-attrs} the attributes of a subflow object are explained.
All lists can be filtered by invoking the \code{FILTER} method with a 
condition expression as a parameter, as shown in example listing 
\ref{lst:rbs-filter-example}.
...
On the wire, HTTP/2 is a framed protocol.

\subsection{Streams and Priorities}

\todo{was sind streams, warum gibt es sie, wie werden sie priorisiert -> zusammenfassen aus http/2 faq}


\todo{diagramm : verschönern oder rauswerfen?}
...
\todo{evtl  merge mit section darüber}

\textcite{han2015mwebmptcp} \todo{ kurze Zusammenfassung }
...
\textcite{nowlan2012unorderedtcp} \todo{ kurze Zusammenfassung}
...
\todo{explain/refer to figures, summarized findings}
...
Figure \ref{fig:annotated-data-path} visualizes the route of an HTTP request 
through the different components of the Nghttpd web server. In the figure, 
the data in the write buffer is tagged with the combination of content 
type and frame type (c.f. Section \ref{http2-frame-types}). Nghttpd uses 
libev for managing its control flow, an event loop library, which means the 
...
\subsection{Recording the Content Type of Streams}
...
\subsection{Retrieving and Buffering Meta Information}
...
\subsection{Passing Meta Information to the Multipath TCP Scheduler}
...
and Ubuntu repositories only supports NPN. Therefore, we built a more recent 
OpenSSL version from source and changed the Nghttpd build configuration to use 
it.
...
found in Annex B.
In downloaded web sites, the content types often can't be determined by the 
file extension. Therefore, the \code{rbs_content_type} parameter can override 
...
script. Register R1 can't be assigned with this parameter as it is used for 
the buffer state information (c.f. listing \ref{lst:nghttpd-write_tls-bufferstate}).
...
We use this protocol from a Python script, \code{measure_web_requests.py} (reproduced in Annex A), to 
...
In simulations on real networks, we chose to keep most third party requests as-is, loading the resource from the original server, especially advertisements, tracking snippets and complex libraries from Content Delivery Networks (CDNs). This is quite realistic, because most of these requests will stay third-party, even if website owners decide to optimize for HTTP/2.
Our virtual networks were completely separated from the internet, therefore all third-party requests were rejected at the network level. However, we downloaded simple JavaScript libraries (consisting of single files) and served them locally, modifying the web page to fetch them from the local server. This is a simple optimization which is realistic to be made by website operators, and allowed us to keep the simulation simple. 
...
prototyping large networks on the constrained resources of a single laptop'', 
by \textcite{Lantz10Mininet}.
...
\todo{description and interpretation of results}

For comparison we use two reference schedulers, one that only transmits on 
the non-backup subflow, which is comparable to single path TCP, and one that 
ignores the backup flag and always uses the fastest available subflow.
...
as well as the browser timings. The results show that with the content-aware 
scheduler we have the same time to first meaningful paint as the reference 
scheduler for both the original and modified Wikipedia homepage, while saving 
60 kB and 1.3 MB of data volume, respectively, on the cellular connection. 
For the CNN artice

\begin{figure}
\centering
	\includegraphics[width=0.8\textwidth]{eva1-rxbytes.pdf}
...
\begin{figure}
\centering
	\includegraphics[width=1\textwidth]{eva1-loadtimes.pdf}
...
\subsection{Evaluation B (``Both lossy'') (TODO!)}

We build an optimized scheduler for situations where multiple lossy network 
connections are available.
When the packet loss raises above a defined threshold, we start sending data 
redundantly. To conserve bandwidth, we only do redundant transmissions for 
latency critical data. The application provides scheduling hints whether the 
data is latency critical or not.
...
\section*{Annex A: measure\_web\_requests.py}

\lstinputlisting[language=Python]{measure_web_requests.py}


\section*{Annex B: prepare\_response}

\begin{lstlisting}[style=C_Code]
