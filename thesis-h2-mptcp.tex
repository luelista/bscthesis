...
\textcite{Ford2008} proposed that endpoint addressing and congestion control 
...
\begin{lstlisting}[style=RBS,label=lst:sched_eval_a,caption={RBS Scheduler Script for Evaluation A (``Slow Wi-Fi'')},numbers=left]
...
\subsection{Evaluation B (``Real World'') (TODO!)}



\subsection{Evaluation C (``Both lossy'') (TODO!)}

We simulated a network with a similar scenario as Evaluation A, but now the 
...
While simulating lossy networks, we noticed that for randomly distributed loss, the results can vary widely between page loads, depending on when during the connection the packet loss occurs. This can be offset by repeating many times and averaging the results. For the results presented below, we loaded the page 100 times with every scheduler.
