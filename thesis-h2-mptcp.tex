...
The syntax of RBS is similar to C, separating statements with the semicolon and using braces for blocks. A visible difference is that keywords are written in upper-case. The control flow elements available are if statements in the form \texttt{IF (condition) \{ ... \} ELSE \{ ... \}}; and a loop to iterate over all elements in a list as \texttt{FOREACH (VAR item IN list) \{ ... \}}. The execution of a script can be stopped at any point with the \code{RETURN} statement. 

\paragraph{Variables and Registers}

Variables are always immutable and can be declared and assigned with \code{VAR myvariable = ...}. They keep their value only during the current script execution. To store state across calls to the scheduler, the registers R1 to R6 can be used. They are always of type integer, therefore it's not possible to store objects or lists of objects permanently. To assign a register, the SET function is called like this: \code{SET(R1, 42)}.
...
Lists provide several methods which facilitate a function approach by using 
...
The HEADERS frame has a dual role as it is also used to open a new stream 
when sent with a new stream identifier. By setting the END\_STREAM flag on
...
HTTP/2 is better suited to use with Multipath TCP, as many small requests are coalesced into one long-running TCP connection.  The overhead of establishing subflows gets less significant, and it can utilize the additional sub-flow capacity better as few
...
schedulers, which is obviously doesn't exist in the wild.
...
It uses the network namespacing feature of Linux to create many virtual network
interfaces, but make only some of them visible to each process.
This allows us to run normal client and server software inside the simulated
environment. %It also has many features we don't use, like the simulation of 
%complex ethernet switches.
...
\begin{table}
	...
	Connection & Packet Loss & Min RTT & Mean RTT & Max RTT & Mdev \\ \midrule

	wifi &	10\%  &  105ms & 148ms  &  398ms &  27ms \\
...
which tunnels all traffic through a VPN 
...
RTT rises to 150ms. The LTE connections RTT varies widely, but can go down to 
30ms, with an average of 80ms.
...
measurements, which is why we did fewer variations than before. We use the 
...
testing is limited. 
The results of the real-world measurements generally confirmed the findings 
of Evaluation A.
After removing 12 failed page loads, we have 120 page loads left, in table 
\ref{eva2-results-tab} they are grouped by scheduler and the mean is 
...
\subsection{Evaluation C (``Both lossy'') (TODO!)}
...
We build an optimized scheduler for browsing the web in situations where multiple network 
connections are available, but they are all lossy. 
The idea is that if the packet loss raises above a defined threshold, we start sending data 
redundantly on both paths. Sending every packet twice will double the bandwidth usage,
...
latency critical data. The application provides scheduling hints to help the scheduler decide whether the 
...
during the connection the packet loss occurs. This can be offset by repeating 
many times and averaging the results. For the results presented below, we 
loaded the page 100 times with every scheduler.
The packet loss on both paths was set to 5 percent.


\todo{doesn't work?  ...what might be the reasons}
...
% multipath scheduling can profit from hints from the application 
