...
is valid for. \code{fill_wb} enqueues and \code{write_tls} dequeues the 
metadata in lockstep with the data. In listing \ref{lst:nghttpd-fill_wb} the 
code for enqueueing the meta information is highlighted.
...
2012 for the SPDY protocol, a precursor of HTTP/2. It is replaced by 
Application Layer Protocol Negotiation, defined in 2014 by RFC 7301. The two 
protocols are very similar, but modern versions of Chrome and Firefox only 
support APLN, while OpenSSL version 1.0.1, which is available in Debian and 
Ubuntu repositories only supports NPN. Therefore, we built a more recent 
...
\texttt{-{}-mptcp-skb-prop}

\paragraph{URL Query Parameters}

\code{rbs_content_type}

\texttt{rbs\_set\_R\textit{<n>}}
...
\paragraph{Scheduler Registers and USER Field}
